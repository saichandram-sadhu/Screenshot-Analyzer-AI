...
import { Activity, Zap, StopCircle, Clock, Cpu, Thermometer, Database } from 'lucide-react';
import { motion } from 'framer-motion';

const PerformanceHUD = ({ metrics, onStop, onUnload, isAnalyzing }) => {
    if (!metrics && !isAnalyzing) return null;

    const tps = parseFloat(metrics?.tps || 0);
        ...
        >
            ...
            <div className="grid grid-cols-3 gap-4 mb-4">
                ...
                <div className="bg-slate-800/50 p-3 rounded-lg border border-slate-700 flex flex-col items-center justify-center relative group">
                    <span className={`text-xs font-mono mb-1 ${metrics?.vram > 0 ? "text-purple-400" : "text-blue-400"}`}>
                        {metrics?.vram > 0
                    ...
                    <div className="flex items-center gap-2 mt-1">
                        <Database className={`w-4 h-4 ${metrics?.vram > 0 ? 'text-purple-400' : 'text-blue-500'}`} />
                        {metrics?.vram === 0 && metrics?.ram > 0 && (
                            <span className="text-[10px] text-slate-500">(CPU Mode)</span>
                        )}

                        {/* Unload Button */}
                        {((metrics?.vram > 0 || metrics?.ram > 0) && !isAnalyzing) && (
                            <button

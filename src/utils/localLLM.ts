interface LocalModel {
    ...
    size: number;
    size_vram?: number;
    gpu_percent?: string; // Added for UI
    digest: string;
    details: {
        format: string;
        family: string;
...
class LocalLLMClient {
    ...
    async checkAvailability(): Promise<AvailabilityResponse> {
        try {
            ...
            if (!response.ok) return { available: false, error: "Service unavailable" };
    ...
    async getRunningInfo(): Promise<LocalModel | null> {
        try {
            ...
            if (data.models?.length > 0) {
                if (data.models?.length > 0) {
                    ...
                    model.gpu_percent = gpuPercent; // Attach to object
    ...
    async listVisionModels(): Promise<LocalModel[]> {
        const status = await this.checkAvailability();
        if (!status.available || !status.models) return [];
        ...
        return status.models.filter((m: LocalModel) =>
